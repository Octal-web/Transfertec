...
use App\Models\Enologia\PaginaIdioma;
...
class PaginasController extends Controller
{
    ...
    public function editarAction(Request $request, $id) {
        if($request->ajax()){
            ...
            if ($response) {
                if ($request->file('img') && $request->file('img')->getError() == 0) {
                    if ($pagina->imagem && File::exists(public_path('eno/content/pages/') . $paginaOriginal->imagem)) {
                        File::delete(public_path('eno/content/pages/') . $paginaOriginal->imagem);
                    }

                    $image = $request->file('img')->move(public_path('eno/content/pages/'), $pagina->imagem);
                }
