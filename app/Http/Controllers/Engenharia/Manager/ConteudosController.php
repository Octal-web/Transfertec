...
use App\Models\Engenharia\Conteudo;
use App\Models\Engenharia\ConteudoIdioma;
...
class ConteudosController extends Controller
{
        ...
        if($request->ajax()){
            ...
            if ($response) {
                ...
                if ($conteudo->parametro->habilitar_img && $request->file('img') && $request->file('img')->getError() == 0) {
                    ...
                    if ($conteudo->imagem && isset($conteudoOriginal) && File::exists('eno/content/display/' . $conteudoOriginal->imagem)) {
                        File::delete('eno/content/display/' . $conteudoOriginal->imagem);
                    }

                    $request->file('img')->move(public_path('eno/content/display/'), $conteudo->imagem);
                    
                }


                if ($conteudo->parametro->habilitar_img_mobile && $request->file('img_mobile') && $request->file('img_mobile')->getError() == 0) {
                    if (isset($conteudoOriginal) && $conteudo->imagem_mobile && File::exists(public_path('eno/content/display/') . $conteudoOriginal->imagem_mobile)) {
                        File::delete(public_path('eno/content/display/') . $conteudoOriginal->imagem_mobile);
                    }

                    $request->file('img')->move(public_path('eno/content/display/') . $conteudo->imagem_mobile);
                }

                if ($conteudo->parametro->habilitar_arquivo && $request->file('arq') && $request->file('arq')->getError() == 0) {
                    if (isset($prev_arquivo) && $conteudo_idioma->arquivo && File::exists(public_path('eno/content/files/') . $prev_arquivo->arquivo)) {
                        File::delete(public_path('eno/content/files/') . $prev_arquivo->arquivo);
                    }

                    $request->file('arq')->storeAs('eno/content/files/', $conteudo_idioma->arquivo);
                }
    ...
    public function baixarArquivo($id) {
        ...
        return Response::download(File::path('eno/content/files/' . $conteudo_idioma->arquivo));
    }
    ...
    public function excluirArquivo(Request $request, $id) {
        if ($request->post()){
            ...
            if ($conteudo->save()) {
                File::delete('eno/content/files/' . $arquivo);
