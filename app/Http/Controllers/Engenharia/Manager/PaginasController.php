...
use App\Models\Engenharia\PaginaIdioma;
...
class PaginasController extends Controller
{
    ...
    public function editarAction(Request $request, $id) {
        if($request->ajax()){
            ...
            if ($response) {
                if ($request->file('img') && $request->file('img')->getError() == 0) {
                    if ($pagina->imagem && File::exists(public_path('eng/content/pages/') . $paginaOriginal->imagem)) {
                        File::delete(public_path('eng/content/pages/') . $paginaOriginal->imagem);
                    }

                    $image = $request->file('img')->move(public_path('eng/content/pages/'), $pagina->imagem);
                }
