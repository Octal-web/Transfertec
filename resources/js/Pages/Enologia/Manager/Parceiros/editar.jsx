...
const Page = () => {
    const { idioma, idiomas, cliente } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(cliente);

    const breadcrumbItems = [
        { label: 'Home', link: 'Enologia.Manager.Home.index' },
        { label: 'Parceiros', link: 'Enologia.Manager.Home.index' },
    ];

    const inputItems = [
        [{ titulo: 'Nome', name: 'nome', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto', max: 120 }],
        [{ titulo: 'Link site', name: 'link', tamanho: 'col-span-12 lg:col-span-8', tipo: 'link', max: 120 }],
        [{ titulo: 'Logo', name: 'img', tamanho: 'col-span-12 lg:col-span-8', tipo: 'imagem', crop: false, largura: 400, altura: 160, imagem: cliente.logo }],
    ];
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Enologia.Manager.Parceiros.atualizar', {id: cliente.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faBuilding} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={cliente.id} />
