...
const Page = () => {
    ...
    useEffect(() => {
        if (isReadyToUpload && data.images.length > 0) {
            post(route('Enologia.Manager.Imagens.novo', { id: conteudo.id }), {
                preserveScroll: true,
                preserveState: false,
            });
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Enologia.Manager.Imagens.ordenar', {id: conteudo.id}), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
    ...
    const breadcrumbItems = [
        { label: conteudo.descricao, link: `Enologia.Manager.${conteudo.controladora}.index` }
    ];
