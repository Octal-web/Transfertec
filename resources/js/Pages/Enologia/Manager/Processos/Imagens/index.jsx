...
import { ImageUploader } from '@/Components/Enologia/Manager/ImageUploader';
import { IndividualImage } from '@/Components/Enologia/Manager/IndividualImage';

const Page = () => {
    // Content
    const { idioma, processo } = usePage().props;

    const [state, setState] = useState(processo.imagens);
    ...
    useEffect(() => {
        if (isReadyToUpload && data.images.length > 0) {
            post(route('Enologia.Manager.Processos.Imagens.novo', { id: processo.id }), {
                preserveScroll: true,
                preserveState: false,
            });
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Manager.Processos.Imagens.ordenar', {id: processo.id}), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
    ...
    const breadcrumbItems = [
        { label: 'Processos', link: 'Enologia.Manager.Home.index' },
        { label: processo.nome, link: 'Enologia.Manager.Processos.editar', params: { id: processo.id } }
    ];
            ...
            <div className="rounded-sm border border-stroke bg-white px-5 py-5 shadow-md">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        >
                            {state.map((image, index) => (
                                <div key={index} className="relative h-full pb-12">
                                    <IndividualImage key={index} individualContent={image} controller="Processos.Imagens" size={{ largura: 735, altura: 800 }} />
                                </div>
                    ...
                    <div className="lg:col-span-1 min-h-[calc(100vh-13rem)]">
                        <ImageUploader onUpload={handleImageUpload} crop={false} size={{ largura: 735, altura: 800 }} />
                    </div>
                </div>
            </div>
        </AdminLayout>
