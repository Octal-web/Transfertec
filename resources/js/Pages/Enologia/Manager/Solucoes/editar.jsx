...
import { faLightbulb, faSave, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
const Page = () => {
    const { idioma, idiomas, solucao } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(solucao);

    const breadcrumbItems = [
        { label: 'Home', link: 'Enologia.Manager.Home.index' },
        { label: 'Soluções', link: 'Enologia.Manager.Home.index' },
    ];
    ...
    const inputItems = [
        ...
        [{ titulo: 'Ícone', name: 'img', tamanho: 'col-span-12 lg:col-span-8', tipo: 'imagem', crop: false, largura: 72, altura: 72, imagem: solucao.icone }],
    ];
    const handleSubmit = (e) => {
        e.preventDefault();
        const idioma_url = new URLSearchParams(window.location.search).get('lang');

        post(route('Enologia.Manager.Solucoes.atualizar', {id: solucao.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faLightbulb} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={solucao.id} />
                ...
                <div className="mt-10">
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="flex items-center justify-end">
                            <Link href={route('Enologia.Manager.Home.index')} className="flex items-center w-fit rounded-lg border border-red-700 text-red-700 px-3 py-2 mr-3 cursor-pointer transition-all hover:bg-red-100">
