...
import { faCog, faSave, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
const Page = () => {
    const { idioma, idiomas, certificacao } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(certificacao);

    const breadcrumbItems = [
        { label: 'Institucional', link: 'Engenharia.Manager.Institucional.index' },
        { label: 'Certificações', link: 'Engenharia.Manager.Institucional.index' },
    ];

    const inputItems = [
        [{ titulo: 'Nome', name: 'nome', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto', max: 120 }],
        [{ titulo: 'Logo', name: 'img', tamanho: 'col-span-12 lg:col-span-8', tipo: 'imagem', crop: false, largura: 250, altura: 250, imagem: certificacao.logo }],
    ];
    const handleSubmit = (e) => {
        e.preventDefault();
        const idioma_url = new URLSearchParams(window.location.search).get('lang');

        post(route('Engenharia.Manager.Certificacoes.atualizar', {id: certificacao.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faCog} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={certificacao.id} />
