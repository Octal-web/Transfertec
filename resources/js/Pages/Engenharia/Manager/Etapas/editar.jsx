...
import { faProjectDiagram, faSave, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
const Page = () => {
    const { idioma, idiomas, etapa } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(etapa);

    const breadcrumbItems = [
        { label: 'Produtos', link: 'Engenharia.Manager.Produtos.index' },
        { label: 'Etapas', link: 'Engenharia.Manager.Produtos.index' },
    ];
    ...
    const inputItems = [
        ...
        [{ titulo: 'Imagem', name: 'img', tamanho: 'col-span-12 lg:col-span-8', tipo: 'imagem', crop: true, largura: 900, altura: 600, imagem: etapa.imagem }],
    ];
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Engenharia.Manager.Etapas.atualizar', {id: etapa.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faProjectDiagram} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={etapa.id} />
                ...
                <div className="mt-10">
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="flex items-center justify-end">
                            <Link href={route('Engenharia.Manager.Produtos.index')} className="flex items-center w-fit rounded-lg border border-red-700 text-red-700 px-3 py-2 mr-3 cursor-pointer transition-all hover:bg-red-100">
