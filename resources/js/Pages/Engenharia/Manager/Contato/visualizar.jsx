...
const Page = () => {
    // Content
    const { idioma, contato } = usePage().props;

    const [isModalOpen, setIsModalOpen] = useState(false);

    const breadcrumbItems = [
        { label: 'Contato', link: 'Engenharia.Manager.Contato.index' },
    ];
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faEnvelope} items={breadcrumbItems} current="Visualizar" idioma={idioma.codigo} />
            ...
            <div className="mb-6 rounded-sm border border-stroke bg-white px-5 py-5 shadow-md">
                ...
                <div className="flex items-center justify-end">
                    <Link href={route('Engenharia.Manager.Contato.index')} className="block relative w-fit rounded-lg border mr-3 border-gray-300 px-3 py-2 cursor-pointer transition-all hover:bg-slate-200">
                ...
                {isModalOpen && <ConfirmModal icon={faTrash} closeModal={closeModal} type="delete" confirm={route('Engenharia.Manager.Contato.excluir', {id: contato.id})} />}
            </div>
        </AdminLayout>
    );
};
