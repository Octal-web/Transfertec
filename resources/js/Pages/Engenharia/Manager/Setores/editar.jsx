...
const Page = () => {
    const { idioma, idiomas, setor } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(setor);

    const breadcrumbItems = [
        { label: 'Home', link: 'Engenharia.Manager.Home.index' },
        { label: 'Setores', link: 'Engenharia.Manager.Home.index' },
    ];
    ...
    const inputItems = [
        ...
        [{ titulo: 'Ícone', name: 'img', tamanho: 'col-span-12 lg:col-span-8', tipo: 'imagem', crop: false, largura: 50, altura: 50, imagem: setor.icone }],
    ];
    const handleSubmit = (e) => {
        e.preventDefault();
        const idioma_url = new URLSearchParams(window.location.search).get('lang');

        post(route('Engenharia.Manager.Setores.atualizar', {id: setor.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faCog} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={setor.id} />
