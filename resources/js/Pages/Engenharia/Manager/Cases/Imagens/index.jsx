...
import { ImageUploader } from '@/Components/Engenharia/Manager/ImageUploader';
import { IndividualImage } from '@/Components/Engenharia/Manager/IndividualImage';

const Page = () => {
    // Content
    const { idioma, caseCliente } = usePage().props;

    const [state, setState] = useState(caseCliente.imagens);
    ...
    useEffect(() => {
        if (isReadyToUpload && data.images.length > 0) {
            post(route('Engenharia.Manager.Cases.Imagens.novo', { id: caseCliente.id }), {
                preserveScroll: true,
                preserveState: false,
            });
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Manager.Cases.Imagens.ordenar', {id: caseCliente.id}), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
            ...
            <div className="rounded-sm border border-stroke bg-white px-5 py-5 shadow-md">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        >
                            {state.map((image, index) => (
                                <div key={index} className="relative h-full pb-12">
                                    <IndividualImage key={index} individualContent={image} controller="Cases.Imagens" size={{ largura: 1600, altura: 900 }} />
                                </div>
                    ...
                    <div className="lg:col-span-1 min-h-[calc(100vh-13rem)]">
                        <ImageUploader onUpload={handleImageUpload} crop={false} />
                    </div>
                </div>
            </div>
        </AdminLayout>
