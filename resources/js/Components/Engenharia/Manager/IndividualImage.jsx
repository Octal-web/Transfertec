...
export const IndividualImage = ({ individualContent, imagensPath, imagensClass, controller, size = { largura: 800, altura: 600 } }) => {
    ...
    const handleCheckboxChange = () => {
        ...
        post(route('Engenharia.Manager.' + controller + '.visibilidade', {id: individualContent.id}), {
            preserveScroll: true,
            onSuccess: (response) => {
    ...
    return (
        <>
            <div className="group relative flex justify-center overflow-hidden border select-none h-full">
                ...
                <ul className="absolute bg-white top-0.5 right-0.5 grid grid-cols-3 items-center transition-all translate-x-full group-hover:translate-x-0 sort-ignore">
            ...
            {isModalOpen && <ConfirmModal icon={faTrash} closeModal={closeModal} type="delete" confirm={route('Engenharia.Manager.' + controller + '.excluir', {id: individualContent.id})} />}
            ...
            {isCropModalOpen && (
                <CropModal 
                    ...
                    cropRoute={route('Engenharia.Manager.' + controller + '.cortar', {id: individualContent.id})}
                    size={size}
                />
            )}
        </>
