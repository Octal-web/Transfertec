...
export const BlockContent = ({ content }) => {
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Engenharia.Manager.' + content.controller + '.ordenar'), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
                ...
                {content.addParametros && content.addParametros.length > 0 ? (
                    <div className="flex">
                        {content.addParametros.map((parameter, index) => (
                            <Link href={route(`Engenharia.Manager.${content.controller}.adicionar`, {tipo: parameter})} className="flex items-center border border-stroke bg-white px-3 py-2 rounded-md transition-all hover:bg-slate-100 ml-2">
                    ...
                    content.addId ? (
                        <Link href={route('Engenharia.Manager.' + content.controller + '.adicionar', { id: content.addId })} className="flex items-center border border-stroke bg-white px-3 py-2 rounded-md transition-all hover:bg-slate-100 ml-2">
                            <FontAwesomeIcon icon={faPlus} className="text-slate-700 mr-2" />
                            {`Adicionar ${content.nome[1]}`}
                        </Link>
                        ) : (
                        <Link href={route('Engenharia.Manager.' + content.controller + '.adicionar')} className="flex items-center border border-stroke bg-white px-3 py-2 rounded-md transition-all hover:bg-slate-100 ml-2">
                            <FontAwesomeIcon icon={faPlus} className="text-slate-700 mr-2" />
                            {`Adicionar ${content.nome[1]}`}
                        </Link>
                    )
                ...
                {content.editavel ? (
                    ...
                    >
                        {state.map((conteudo, index) => (
                            <div key={index} className="relative rounded-md border border-stroke p-4 shadow-sm select-none before:content-[''] before:absolute before:top-0 before:left-0 before:bg-eng-primary before:w-full before:h-1 before:rounded-t-md">
                                <IndividualContent 
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 sm:gap-x-6 gap-y-4 sm:gap-y-8">
                        {content.conteudos.map((conteudo, index) => (
                            <div key={index} className="relative rounded-md border border-stroke p-4 shadow-sm before:content-[''] before:absolute before:top-0 before:left-0 before:bg-eng-primary before:w-full before:h-1 before:rounded-t-md">
                                <IndividualContent 
                                    key={index}
                                    individualContent={conteudo}
                                    imagensPath={content.imagensPath}
                    ...
                    content.addParametros && content.addParametros.length > 0 ? (
                        <div className="flex">
                            {content.addParametros.map((parameter, index) => (
                                <Link
                                    key={index}
                                    href={route(`Engenharia.Manager.${content.controller}.adicionar`, { tipo: parameter })}
                        ...
                        <Link
                            href={route(`Engenharia.Manager.${content.controller}.adicionar`)}
                            className="flex items-center border border-stroke bg-white px-3 py-2 rounded-md transition-all hover:bg-slate-100 ml-2"
                        >
                            <FontAwesomeIcon icon={faPlus} className="text-slate-700 mr-2" />
                            {`Adicionar ${content.nome[1]}`}
