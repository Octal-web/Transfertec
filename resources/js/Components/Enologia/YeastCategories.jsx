...
export const YeastCategories = ({ categories, selectedCategory, setSelectedCategory, selectedSubcategory, setSelectedSubcategory, loading }) => {
    ...
    return (
        <div className="md:col-span-7 flex max-md:pt-2">
            <div className="hidden md:flex w-full">
                <div className="w-1/2 py-4 px-6 2xl:p-6 border-r border-gray-700">
                ...
                {selectedCategory && selectedCategory.subcategorias && selectedCategory.subcategorias.length > 0 ? (
                    <div className="w-1/2 p-6 flex flex-col">
                            ...
                            {selectedCategory.subcategorias.map((subcategory) => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 text-gray-400 hover:text-white text-lg hover:bg-neutral-800 transition-colors duration-200 ${
                                        selectedCategory.subcategorias.length <= 1 && subcategory.nome === 'Todos' ? 'hidden' : ''
                                    }`}
                                >
                                    {subcategory.nome}
                                </button>
                        ...
                        {selectedCategory.marcas && selectedCategory.marcas.length > 0 ? (
                            <div className={`grid grid-cols-2 gap-4 pt-4 ${
                                selectedCategory.subcategorias.length <= 1 && selectedCategory.subcategorias[0].nome === 'Todos' ? '' : ' mt-auto'
                            }`}>
                                {selectedCategory.marcas.map((brand, index) => (
                                    <a key={index} href={brand.link} target="_blank" className="transition-all hover:scale-90">
            ...
            <div className="md:hidden w-full">
                {mobileView === 'categories' ? (
                    ...
                    <div className="p-4">
                        ...
                        {selectedCategory && selectedCategory.marcas && selectedCategory.marcas.length > 0 && (
                            <div className="border-t border-gray-700 pt-4">
                                <h4 className="text-lg font-semibold text-white mb-4">Marcas</h4>
                                <div className="grid grid-cols-2 gap-4">
                                    {selectedCategory.marcas.map((brand, index) => (
                                        <a key={index} href={brand.link} target="_blank" className="transition-all hover:scale-95 active:scale-90">
                                            <img src={brand.logo} alt={brand.nome} className="w-full h-auto" />
                                        </a>
                                    ))}
                                </div>
                            </div>
