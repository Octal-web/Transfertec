    ...
    return (
        <div className="relative">
                ...
                {slides.map((slide, index) => (
                    <SwiperSlide key={slide.id}>
                        <div className="relative z-[1] h-[calc(100vh_-_145px)] flex items-center">
                            ...
                            <div className="absolute inset-0 bg-[#471901] mix-blend-color-dodge opacity-40" />

                            <div className="container max-w-large h-full">
                                <div
                                    className={`flex flex-col relative w-full h-full max-md:pb-20 max-md:w-[90%] max-md:ml-0 max-2xl:ml-6 xl:w-1/2 max-w-[574px] justify-end pb-16 2xl:pb-36 transition-opacity duration-1000 ease-in-out z-[1] ${
                                        activeIndex === index
                                ...
                                >
                                    {slide.titulo && (
                                        <h2 className="text-4xl 2xl:text-5xl font-medium text-white leading-[1.1] text-balance mb-4 md:mb-8" dangerouslySetInnerHTML={{ __html: slide.titulo }} />
                                    )}
                                    {slide.descricao && (
                                        <div className="font-secondary text-white text-xl font-light max-sm:leading-tight max-w-lg">
                                            <p>{slide.descricao}</p>
                                        </div>
                                    )}
                                    {slide.link && (
                                        <a href={slide.link} className="bg-eno-primary w-fit px-8 py-3 rounded-lg text-xl md:text-3xl text-white font-medium tracking-tight transition-all hover:scale-105 hover:shadow" target="_blank" rel="noopener noreferrer">{slide.texto_botao}</a>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            <div className="relative container max-w-large">
                <div className="absolute bottom-8 md:bottom-20 2xl:bottom-44 max-md:w-full left-0 z-10 opacity-80 max-md:ml-0 max-2xl:ml-8">
                    <div className="container max-w-large">
                        <div className="flex md:flex-col items-center md:justify-between">
                            ...
                            <div
                                ref={paginationRef}
                                className="flex md:flex-col items-center max-md:!space-x-4 md:!space-y-4 max-md:mx-5 md:my-5 max-md:!w-fit"
                            ></div>
