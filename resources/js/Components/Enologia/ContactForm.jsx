...
export const ContactForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Enologia.Contato.enviar'), {
            preserveScroll: true,
            onSuccess: (page) => {
                setLoading(false);
            },
    ...
    return (
        <section className="relative">
            <div className="container max-w-medium">
                <h1 className="text-5xl 2xl:text-[78px] text-neutral-900 font-light leading-none mb-3">Preencha os<br />campos abaixo</h1>
                <h4 className="font-secondary text-xl 2xl:text-2xl text-neutral-900 mb-8 2xl:mb-12">e entraremos em contato com você.</h4>
                ...
                >
                    <div className="mb-3 2xl:mb-4 flex gap-3 md:gap-6 flex-col lg:flex-row">
                        <div className="w-full">
                            <input type="text" name="nome" value={data.nome} onChange={handleChange} placeholder="Nome completo" className="w-full h-12 px-4 border-1 border-neutral-900 rounded focus:outline-none focus:ring-0 focus:border-eno-secondary focus:shadow-inner transition-colors duration-200 placeholder:text-gray-500 placeholder:text-opacity-70"  />
                        ...
                        <div className="my-2 w-full">
                            <label className="flex items-center">
                                <label className="relative flex">
                                    ...
                                    <span className="peer-checked:content-[''] peer-checked:absolute peer-checked:inset-1 rounded peer-checked:bg-eno-primary" />
                                </label>

                                <span className="max-sm:text-sm text-neutral-600 ml-2">Li e concordo com a <Link href={route('Enologia.Politicas.privacidade')} className="underline" target="_blank" rel="noopener noreferrer">Política de Privacidade</Link>.</span>
                            </label>
                    ...
                    <div className="mb-3 2xl:mb-4 flex gap-3 md:gap-6 flex-col lg:flex-row">
                        <button
                            type="submit"
                            className="relative px-7 py-3 w-fit lg:text-xl 2xl:text-2xl bg-eno-primary font-secondary text-white text-center font-semibold rounded-xl whitespace-nowrap transition-all hover:bg-eno-secondary"
                        >
                            {!loading ? (
                                'Enviar mensagem'
                            ) : (
            ...
            {isSuccessful && (
                <div className={`absolute inset-0 flex flex-col items-center justify-center bg-white pointer-events-none animate-fade-in-down`}>
                    ...
                    <h2 className="text-neutral-900 text-4xl text-center font-semibold mt-4 mb-2">Sucesso!</h2>
                    <h4 className="font-secondary text-neutral-900 text-2xl text-center">Sua mensagem foi enviada.</h4>
                </div>
            )}
        </section>
    );
