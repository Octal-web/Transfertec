...
export const IndividualItem = ({ individualContent, imagensPath, imagensClass, controller, index, edit }) => {
    ...
    const handleCheckboxChange = () => {
        ...
        post(route('Enologia.Manager.' + controller + '.visibilidade', {id: individualContent.id}), {
            preserveScroll: true,
            onSuccess: (response) => {
    ...
    return (
        <tr ref={rowRef} className="bg-slate-50">
            ...
            <td className="border max-sm:text-center px-2 md:px-4 py-2 w-1/6 sort-ignore" width={`${cellWidths[3] || 'auto'}`}>
                ...
                {isModalOpen && <ConfirmModal icon={faTrash} closeModal={closeModal} type="delete" confirm={route('Enologia.Manager.' + controller + '.excluir', {id: individualContent.id})} />}
            </td>
        </tr>
    );
};
